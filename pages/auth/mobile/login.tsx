...
const LoginOKComponent : React.FC = () => {
    ...
    useEffect(() => {
        if (status === 'authenticated' && !sessionSent) {
            if (query.error) {
                setError(true);
            }
            else if(query.data) {
                ...
                // @ts-ignore
                devices = query.data.filter((device) => device.owner.id === user.id).map((device) => ({
                    ...device,
                ...
                if (window.flutter_inappwebview) {
                    // @ts-ignore
                    window.flutter_inappwebview.callHandler('mobileSessionHandler', JSON.stringify({ user, devices }));
                    setSessionSent(true);
                }
            }
        }
    }, [status, sessionSent, query]);
